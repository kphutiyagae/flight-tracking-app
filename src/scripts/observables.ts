import {
  Subject, iif, from, Observable, of,
} from 'rxjs';
...
function isIFlightApiResponse(
  response: IFlightAPIResponse |
  { error: boolean, message: string },
): response is IFlightAPIResponse {
    ...
    if (isIFlightApiResponse(data)) {
      data?.states.map((stateVector) => {
        ...
        if (flightData.callsign
          && (flightData.latitude && flightData.longitude)) flightArray.push(flightData);
      });
      return flightArray.slice(0, 20);
    }
    return flightArray;
  }),
  map((flightsArray) => flightsArray),
  ...
  map((data) => {
    ...
    if (isIFlightApiResponse(data)) {
      data?.states.map((stateVector) => {
        ...
        if (flightData.callsign
          && (flightData.latitude && flightData.longitude)) flightA.push(flightData);
      });
      return flightA.slice(0, 20);
    } return flightA;
  }),
  map((flightsArray: IFlight[]) => flightsArray),
  catchError((e: IofError) => {
    const error: IofError = { error: true, message: `Error ${e.message}` };
    return of(error);
  }),
...
cachedOrRequestedFlightData$.subscribe((flightDataArray) => flightData$.next(flightDataArray));
